...
if (logsTypes and logsTypes == "FILES"):
    from logsInFile import printLogs, printModifs
elif (logsTypes and logsTypes == "DB"):
    from logsInDB import printLogs, printModifs
else :
    from noLogs import printLogs, printModifs
...
class Database:
    def __init__(self):
        ...
        self.cursor = self.connection.cursor(cursor_class=MyCursor)
    def close(self):
        self.connection.close()

class MyCursor(CMySQLCursor):
    ...
    def executemany(self, operation, seq_params):
        for params in seq_params:
            query = operation % self._process_params(params)
            printDB(query)
        return super().executemany(operation, seq_params)

    def _process_params(self, params):
        return tuple(self._process_param(param) for param in params)

    def _process_param(self, param):
        if isinstance(param, str):
            return f"'{self.connection.converter.escape(param)}'"
        return f"'{param}'"

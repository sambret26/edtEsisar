...
def deleteEvent(area, calId, retry=5):
    ...
    try:
        ...
        printLogs(logs.CAL, logs.WARN, "Retry delete ({}) ({})".format(retry, calId))
...
def createEvent(area, event, retry=5):
    if retry ==0:
        printLogs(logs.CAL, logs.ERROR, "Error creating {} on {}".format(event["Subject"], event["Start"]))
        return False
    try:
        calendarId = DB.getCalendarId(area)
...
def updateEvent(area, event, retry=5):
    ...
    try:
        ...
        printLogs(logs.CAL, logs.WARN, "Retry update ({}) ({})".format(retry, calId))

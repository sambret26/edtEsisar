...
def _build_ssl_context(
  ...
  context.verify_mode = (ssl.CERT_NONE if disable_ssl_certificate_validation
...
def _parse_cache_control(headers):
  ...
  if "cache-control" in headers:
    ...
    parts_with_args = [
      tuple([x.strip().lower() for x in part.split("=", 1)]) for part in parts
      if -1 != part.find("=")
    ]
    parts_wo_args = [(name.strip().lower(), 1) for name in parts
                     if -1 == name.find("=")]
...
def _parse_www_authenticate(headers, headername="www-authenticate"):
  ...
  if headername in headers:
    try:
      ...
      www_auth = (USE_WWW_AUTH_STRICT_PARSING and WWW_AUTH_STRICT
                  or WWW_AUTH_RELAXED)
      while authenticate:
...
def _cnonce():
  dig = _md5(
    ("%s:%s" %
     (time.ctime(), ["0123456789"[random.randrange(0, 9)]
                     for i in range(20)])).encode("utf-8")).hexdigest()
  return dig[:16]
...
def _wsse_username_token(cnonce, iso_now, password):
  return base64.b64encode(
    _sha(("%s%s%s" %
          (cnonce, iso_now, password)).encode("utf-8")).digest()).strip()
...
class DigestAuthentication(Authentication):
  ...
  def __init__(self, credentials, host, request_uri, headers, response,
    ...
    if self.challenge["algorithm"] != "MD5":
      raise UnimplementedDigestAuthOptionError(
        _("Unsupported value for algorithm: %s." %
...
class HmacDigestAuthentication(Authentication):
  ...
  def __init__(self, credentials, host, request_uri, headers, response,
    ...
    if self.challenge["pw-algorithm"] not in ["SHA-1", "MD5"]:
      raise UnimplementedHmacDigestAuthOptionError(
        _("Unsupported value for pw-algorithm: %s." %
...
"""
  ...
  def _request(
    ...
    if (self.follow_all_redirects or method in self.safe_methods
...
a string that contains the response entity body.
    ...
    try:
      ...
      if (method in self.optimistic_concurrency_methods and self.cache
          and "etag" in info and not self.ignore_etag
      ...
      if (self.cache and cached_value
        ...
        if "-x-permanent-redirect-url" in info:
          ...
          if entry_disposition == "STALE":
            if ("etag" in info and not self.ignore_etag
